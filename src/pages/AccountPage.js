      ...
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            ...
            <Link
              lineHeight={'145.23%'}
              href={'https://web3-explorer-mesos.thestratos.org/address/' + account}
              target="_blank"
            >
              {' '}
              {account?.slice(0, 42)}{' '}
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WSTOS') {
                        p.pair.token1.symbol = 'STOS'
                      }
                      if (p.pair.token0.symbol === 'WSTOS') {
                        p.pair.token0.symbol = 'STOS'
                      }
                      return (
